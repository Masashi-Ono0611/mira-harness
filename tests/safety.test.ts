...
test("a plain one-shot Confirm callback IS found", () => {
  const f = findConfirmButton(
    res([m(1, "Model: X · Cost 30 🪙 · Confirm?", [{ text: "✅ Confirm", callbackData: b64("go") }])]),
  );
  assert.ok(f);
...
test('"Always yes" (persistent) is NEVER pressed', () => {
  assert.equal(
...
test("a url button (no callbackData) is not actionable", () => {
  assert.equal(findConfirmButton(res([m(1, "Confirm?", [{ text: "✅ Confirm", url: "https://x" }])])), undefined);
});

test("a wallet/transfer context message is skipped entirely", () => {
  assert.equal(
...
test("an OAuth/Connect button is refused", () => {
  assert.equal(
...
test("a non-confirm callback (e.g. Cancel) is ignored", () => {
  assert.equal(findConfirmButton(res([m(1, "options", [{ text: "Cancel", callbackData: b64("n") }])])), undefined);
});

test('"Generate" counts as confirm-ish', () => {
