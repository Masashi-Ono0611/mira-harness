/**
 * Unit tests for capture.ts — pure, no network, no Telegram. Run: `bun test`.
 *
...
test("links: a text_url (source link) + a bare url", () => {
...
test("non-inline markup yields no buttons", () => {
  assert.deepEqual(extractButtons({ className: "ReplyKeyboardMarkup" } as unknown as Api.TypeReplyMarkup), []);
});

test("media: photo, document(video mime -> kind=video), webpage url", () => {
  assert.equal(extractMedia({ className: "MessageMediaPhoto" } as unknown as Api.TypeMessageMedia)?.kind, "photo");
  const doc = {
  ...
  const mediaResult = extractMedia(doc as unknown as Api.TypeMessageMedia);
  assert.equal(mediaResult?.kind, "video");
  assert.equal(mediaResult?.fileName, "clip.mp4");
...
test("extractMessage: integration + editCount passthrough", () => {
  const msg = {
    id: 99,
    message: "here you go",
